#include "main.h"
#include <opencv2/core/core.hpp>
#include <opencv2/highgui/highgui.hpp>
#include <opencv2/video/background_segm.hpp>

#include <stdio.h>

#include <iostream>
#include <sstream>
#include <fstream>
#include "opencv2/video/tracking.hpp"
#include "opencv2/highgui/highgui.hpp"

#define dis 100 
using namespace cv;
using namespace std;
Mat frame; //current frame
Mat fgMaskMOG; //fg mask generated by MOG method
Mat fgMaskMOG2; //fg mask fg mask generated by MOG2 method
Ptr<BackgroundSubtractor> pMOG; //MOG Background subtractor
Ptr<BackgroundSubtractor> pMOG2; //MOG2 Background subtractor
int keyboard;
void help();
void processVideo(char* videoFilename);
void processImages(char* firstFrameFilename);
double speed[100];


inline bool isFlowCorrect(Point2f u)
{
    return !cvIsNaN(u.x) && !cvIsNaN(u.y) && fabs(u.x) < 1e9 && fabs(u.y) < 1e9;
}

static Vec3b computeColor(float fx, float fy)
{
    static bool first = true;

    // relative lengths of color transitions:
    // these are chosen based on perceptual similarity
    // (e.g. one can distinguish more shades between red and yellow
    //  than between yellow and green)
    const int RY = 15;
    const int YG = 6;
    const int GC = 4;
    const int CB = 11;
    const int BM = 13;
    const int MR = 6;
    const int NCOLS = RY + YG + GC + CB + BM + MR;
    static Vec3i colorWheel[NCOLS];

    if (first)
    {
        int k = 0;

        for (int i = 0; i < RY; ++i, ++k)
            colorWheel[k] = Vec3i(255, 255 * i / RY, 0);

        for (int i = 0; i < YG; ++i, ++k)
            colorWheel[k] = Vec3i(255 - 255 * i / YG, 255, 0);

        for (int i = 0; i < GC; ++i, ++k)
            colorWheel[k] = Vec3i(0, 255, 255 * i / GC);

        for (int i = 0; i < CB; ++i, ++k)
            colorWheel[k] = Vec3i(0, 255 - 255 * i / CB, 255);

        for (int i = 0; i < BM; ++i, ++k)
            colorWheel[k] = Vec3i(255 * i / BM, 0, 255);

        for (int i = 0; i < MR; ++i, ++k)
            colorWheel[k] = Vec3i(255, 0, 255 - 255 * i / MR);

        first = false;
    }

    const float rad = sqrt(fx * fx + fy * fy);
    const float a = atan2(-fy, -fx) / (float)CV_PI;

    const float fk = (a + 1.0f) / 2.0f * (NCOLS - 1);
    const int k0 = static_cast<int>(fk);
    const int k1 = (k0 + 1) % NCOLS;
    const float f = fk - k0;

    Vec3b pix;

    for (int b = 0; b < 3; b++)
    {
        const float col0 = colorWheel[k0][b] / 255.f;
        const float col1 = colorWheel[k1][b] / 255.f;

        float col = (1 - f) * col0 + f * col1;

        if (rad <= 1)
            col = 1 - rad * (1 - col); // increase saturation with radius
        else
            col *= .75; // out of range

        pix[2 - b] = static_cast<uchar>(255.f * col);
    }

    return pix;
}

static void drawOpticalFlow(const Mat_<Point2f>& flow, Mat& dst, float maxmotion = -1)
{
    dst.create(flow.size(), CV_8UC3);
    dst.setTo(Scalar::all(0));

    // determine motion range:
    float maxrad = maxmotion;

    if (maxmotion <= 0)
    {
        maxrad = 1;
        for (int y = 0; y < flow.rows; ++y)
        {
            for (int x = 0; x < flow.cols; ++x)
            {
                Point2f u = flow(y, x);

                if (!isFlowCorrect(u))
                    continue;

                maxrad = max(maxrad, sqrt(u.x * u.x + u.y * u.y));
            }
        }
    }

    for (int y = 0; y < flow.rows; ++y)
    {
        for (int x = 0; x < flow.cols; ++x)
        {
            Point2f u = flow(y, x);

            if (isFlowCorrect(u))
                dst.at<Vec3b>(y, x) = computeColor(u.x / maxrad, u.y / maxrad);
        }
    }
}


static void writeOpticalFlowToFile(const Mat_<Point2f>& flow, const string& fileName)
{
    static const char FLO_TAG_STRING[] = "PIEH";

    ofstream file(fileName.c_str(), ios_base::binary);

    file << FLO_TAG_STRING;

    file.write((const char*) &flow.cols, sizeof(int));
    file.write((const char*) &flow.rows, sizeof(int));

    for (int i = 0; i < flow.rows; ++i)
    {
        for (int j = 0; j < flow.cols; ++j)
        {
            const Point2f u = flow(i, j);

            file.write((const char*) &u.x, sizeof(float));
            file.write((const char*) &u.y, sizeof(float));
        }
    }
}



void help()
{
	cout
		<< "--------------------------------------------------------------------------" << endl
		<< "This program shows how to use background subtraction methods provided by " << endl
		<< " OpenCV. You can process both videos (-vid) and images (-img)." << endl
		<< endl
		<< "Usage:" << endl
		<< "./bs {-vid <video filename>|-img <image filename>}" << endl
		<< "for example: ./bs -vid video.avi" << endl
		<< "or: ./bs -img /data/images/1.png" << endl
		<< "--------------------------------------------------------------------------" << endl
		<< endl;
}
int main(int argc, char* argv[])
{
    VideoCapture cap("video.avi"); // open the video file for reading

    if ( !cap.isOpened() )  // if not success, exit program
    {
         cout << "Cannot open the video file" << endl;
         return -1;
    }

    //cap.set(CV_CAP_PROP_POS_MSEC, 300); //start the video at 300ms

    double fps = cap.get(CV_CAP_PROP_FPS); //get the frames per seconds of the video

     cout << "Frame per seconds : " << fps << endl;

    namedWindow("Frame");
	namedWindow("FG Mask MOG");
	namedWindow("FG Mask MOG 2");
	pMOG = new BackgroundSubtractorMOG(); //MOG approach
	pMOG2 = new BackgroundSubtractorMOG2(); 
	 //namedWindow("MyVideo",CV_WINDOW_AUTOSIZE); //create a window called "MyVideo"

    while(1)
    {
        Mat frame;

        bool bSuccess = cap.read(frame); // read a new frame from video

if (!bSuccess) //if not success, break loop
{
                        cout << "Cannot read the frame from video file" << endl;
break;
}

		pMOG->operator()(frame, fgMaskMOG);
		pMOG2->operator()(frame, fgMaskMOG2);
		//get the frame number and write it on the current frame
		stringstream ss;
		rectangle(frame, cv::Point(10, 2), cv::Point(100, 20),
			cv::Scalar(255, 255, 255), -1);
		ss << cap.get(CV_CAP_PROP_POS_FRAMES);
		string frameNumberString = ss.str();
		putText(frame, frameNumberString.c_str(), cv::Point(15, 15),
			FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0));
		//show the current frame and the fg masks
		imshow("Frame", frame);
		imshow("FG Mask MOG", fgMaskMOG);
		imshow("FG Mask MOG 2", fgMaskMOG2);
		//get the input from the keyboard
		keyboard = waitKey(30);

   //     imshow("MyVideo", frame); //show the frame in "MyVideo" window

     /*   if(waitKey(30) == 27) //wait for 'esc' key press for 30 ms. If 'esc' key is pressed, break loop
{
                cout << "esc key is pressed by user" << endl;
break; 
}*/
		string imageToSave = "output_MOG_" + frameNumberString + ".png";
bool saved = imwrite(imageToSave, fgMaskMOG);
if(!saved) {
  cerr << "Unable to save " << imageToSave << endl;
}    

    }
	int kkk=0;
    
    //int j;
    
    
    
/*
    if (argc < 3)
    {
        cerr << "Usage : " << argv[0] << "<frame0> <frame1> [<output_flow>]" << endl;
        return -1;
    }
    */
  //  int j;
//    for(j = 1 ;j < 374 ; j++){
    Mat frame0 = imread("output_MOG_1.png", IMREAD_GRAYSCALE);
    Mat frame1 = imread("output_MOG_60.png", IMREAD_GRAYSCALE);

    /*
    if (frame0.empty())
    {
        cerr << "Can't open image ["  << argv[1] << "]" << endl;
        return -1;
    }
    if (frame1.empty())
    {
        cerr << "Can't open image ["  << argv[2] << "]" << endl;
        return -1;
    }

    if (frame1.size() != frame0.size())
    {
        cerr << "Images should be of equal sizes" << endl;
        return -1;
    }
    */
    Mat_<Point2f> flow;
    Ptr<DenseOpticalFlow> tvl1 = createOptFlow_DualTVL1();

    const double start = (double)getTickCount();
    tvl1->calc(frame0, frame1, flow);
    const double timeSec = (getTickCount() - start) / getTickFrequency();
	speed[kkk++]=dis/timeSec;
    cout << "calcOpticalFlowDual_TVL1 : " << timeSec << " sec" << endl;

    Mat out;
    drawOpticalFlow(flow, out);

   // if (argc == 4)
    //    writeOpticalFlowToFile(flow, argv[3]);

    imshow("Flow", out);
    waitKey(160);
//}
















	




	Mat frame2 = imread("output_MOG_61.png", IMREAD_GRAYSCALE);
    Mat frame3 = imread("output_MOG_120.png", IMREAD_GRAYSCALE);

    
    /*if (frame2.empty())
    {
        cerr << "Can't open image ["  << argv[1] << "]" << endl;
        return -1;
    }
    if (frame3.empty())
    {
        cerr << "Can't open image ["  << argv[2] << "]" << endl;
        return -1;
    }

    if (frame3.size() != frame0.size())
    {
        cerr << "Images should be of equal sizes" << endl;
        return -1;
    }
    */
    Mat_<Point2f> flow1;
    Ptr<DenseOpticalFlow> tvl2 = createOptFlow_DualTVL1();

    const double start1 = (double)getTickCount();
    tvl2->calc(frame2, frame3, flow1);
    const double timeSec1 = (getTickCount() - start1) / getTickFrequency();
    cout << "calcOpticalFlowDual_TVL1 : " << timeSec1 << " sec" << endl;
	speed[kkk++]=dis/timeSec1;
    Mat out1;
    drawOpticalFlow(flow1, out1);

   // if (argc == 4)
    //    writeOpticalFlowToFile(flow, argv[3]);

    imshow("Flow1", out1);
    waitKey(120);
//}





	
	Mat frame4 = imread("output_MOG_121.png", IMREAD_GRAYSCALE);
    Mat frame5 = imread("output_MOG_180.png", IMREAD_GRAYSCALE);

    
    /*if (frame2.empty())
    {
        cerr << "Can't open image ["  << argv[1] << "]" << endl;
        return -1;
    }
    if (frame3.empty())
    {
        cerr << "Can't open image ["  << argv[2] << "]" << endl;
        return -1;
    }

    if (frame3.size() != frame0.size())
    {
        cerr << "Images should be of equal sizes" << endl;
        return -1;
    }
    */
    Mat_<Point2f> flow2;
    Ptr<DenseOpticalFlow> tvl3 = createOptFlow_DualTVL1();

    const double start2 = (double)getTickCount();
    tvl3->calc(frame4, frame5, flow2);
    const double timeSec2 = (getTickCount() - start2) / getTickFrequency();
    cout << "calcOpticalFlowDual_TVL1 : " << timeSec2 << " sec" << endl;
	speed[kkk++]=dis/timeSec2;
    Mat out2;
    drawOpticalFlow(flow2, out2);

   // if (argc == 4)
    //    writeOpticalFlowToFile(flow, argv[3]);

    imshow("Flow2", out2);
    waitKey(120);
//}




	Mat frame6 = imread("output_MOG_181.png", IMREAD_GRAYSCALE);
    Mat frame7 = imread("output_MOG_240.png", IMREAD_GRAYSCALE);

   
    Mat_<Point2f> flow3;
    Ptr<DenseOpticalFlow> tvl4 = createOptFlow_DualTVL1();

    const double start3 = (double)getTickCount();
    tvl4->calc(frame6, frame7, flow3);
    const double timeSec3 = (getTickCount() - start3) / getTickFrequency();
    cout << "calcOpticalFlowDual_TVL1 : " << timeSec3 << " sec" << endl;
	speed[kkk++]=dis/timeSec3;
    Mat out3;
    drawOpticalFlow(flow3, out3);

   
    imshow("Flow3", out3);
    waitKey(120);
//}



	Mat frame8 = imread("output_MOG_241.png", IMREAD_GRAYSCALE);
    Mat frame9 = imread("output_MOG_300.png", IMREAD_GRAYSCALE);

    
   
    Mat_<Point2f> flow4;
    Ptr<DenseOpticalFlow> tvl5 = createOptFlow_DualTVL1();

    const double start4 = (double)getTickCount();
    tvl5->calc(frame8, frame9, flow4);
    const double timeSec4 = (getTickCount() - start4) / getTickFrequency();
    cout << "calcOpticalFlowDual_TVL1 : " << timeSec4 << " sec" << endl;
	speed[kkk++]=dis/timeSec4;
    Mat out4;
    drawOpticalFlow(flow4, out4);

   

    imshow("Flow4", out4);
    waitKey(120);
//}



	Mat frame10 = imread("output_MOG_301.png", IMREAD_GRAYSCALE);
    Mat frame11 = imread("output_MOG_360.png", IMREAD_GRAYSCALE);

    
   
    Mat_<Point2f> flow5;
    Ptr<DenseOpticalFlow> tvl6 = createOptFlow_DualTVL1();

    const double start5 = (double)getTickCount();
    tvl6->calc(frame10, frame11, flow5);
    const double timeSec5 = (getTickCount() - start5) / getTickFrequency();
    cout << "calcOpticalFlowDual_TVL1 : " << timeSec5 << " sec" << endl;
	speed[kkk++]=dis/timeSec5;
    Mat out5;
    drawOpticalFlow(flow5, out5);

   

    imshow("Flow5", out5);
    waitKey(120);










	double sum =0;
	for(int i=0;i<kkk;i++){
		sum=sum+speed[i];
	}
	cout << " AVERAGE VELOCITY IS = " << (sum/kkk)*(5.0/18.0) <<"km/hr"<< endl;








 
	waitKey ();
cap.release();

    return 0;

}
void processImages(char* fistFrameFilename) {
	//read the first file of the sequence
	frame = imread(fistFrameFilename);
	if (!frame.data){
		//error in opening the first image
		cerr << "Unable to open first image frame: " << fistFrameFilename << endl;
		exit(EXIT_FAILURE);
	}
	//current image filename
	string fn(fistFrameFilename);
	//read input data. ESC or 'q' for quitting
	while ((char)keyboard != 'q' && (char)keyboard != 27){
		//update the background model
		//ALSO HERE!!!!
		pMOG->operator()(frame, fgMaskMOG);
		pMOG2->operator()(frame, fgMaskMOG2);
		//get the frame number and write it on the current frame
		size_t index = fn.find_last_of("/");
		if (index == string::npos) {
			index = fn.find_last_of("\\");
		}
		size_t index2 = fn.find_last_of(".");
		string prefix = fn.substr(0, index + 1);
		string suffix = fn.substr(index2);
		string frameNumberString = fn.substr(index + 1, index2 - index - 1);
		istringstream iss(frameNumberString);
		int frameNumber = 0;
		iss >> frameNumber;
		rectangle(frame, cv::Point(10, 2), cv::Point(100, 20),
			cv::Scalar(255, 255, 255), -1);
		putText(frame, frameNumberString.c_str(), cv::Point(15, 15),
			FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0));
		//show the current frame and the fg masks
		imshow("Frame", frame);
		imshow("FG Mask MOG", fgMaskMOG);
		imshow("FG Mask MOG 2", fgMaskMOG2);
		//get the input from the keyboard
		keyboard = waitKey(30);
		//search for the next image in the sequence
		ostringstream oss;
		oss << (frameNumber + 1);
		string nextFrameNumberString = oss.str();
		string nextFrameFilename = prefix + nextFrameNumberString + suffix;
		//read the next frame
		frame = imread(nextFrameFilename);
		if (!frame.data){
			//error in opening the next image in the sequence
			cerr << "Unable to open image frame: " << nextFrameFilename << endl;
			exit(EXIT_FAILURE);
		}
		//update the path of the current frame
		fn.assign(nextFrameFilename);
	}
}
void processVideo(char* videoFilename) {
	//create the capture object
	VideoCapture capture(videoFilename);
	if (!capture.isOpened()){
		//error in opening the video input
		cerr << "Unable to open video file: " << videoFilename << endl;
		exit(EXIT_FAILURE);
	}
	//read input data. ESC or 'q' for quitting
	while ((char)keyboard != 'q' && (char)keyboard != 27){
		//read the current frame
		if (!capture.read(frame)) {
			cerr << "Unable to read next frame." << endl;
			cerr << "Exiting..." << endl;
			exit(EXIT_FAILURE);
		}
		//update the background model
		//AND HERE!!!
		pMOG->operator()(frame, fgMaskMOG);
		pMOG2->operator()(frame, fgMaskMOG2);
		//get the frame number and write it on the current frame
		stringstream ss;
		rectangle(frame, cv::Point(10, 2), cv::Point(100, 20),
			cv::Scalar(255, 255, 255), -1);
		ss << capture.get(CV_CAP_PROP_POS_FRAMES);
		string frameNumberString = ss.str();
		putText(frame, frameNumberString.c_str(), cv::Point(15, 15),
			FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0));
		//show the current frame and the fg masks
		imshow("Frame", frame);
		imshow("FG Mask MOG", fgMaskMOG);
		imshow("FG Mask MOG 2", fgMaskMOG2);
		//get the input from the keyboard
		keyboard = waitKey(30);
	}
	//delete capture object
	capture.release();
}